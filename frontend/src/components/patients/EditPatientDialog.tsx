...
export default function EditPatientDialog({ open, patient, saving, onClose, onSave }: Props) {
  const [values, setValues] = useState<PatientCreateRequest>({ firstName: "", lastName: "", email: "", phone: "", diagnosis: "" });
  useEffect(() => { if (patient) setValues({ firstName: patient.firstName, lastName: patient.lastName, email: patient.email, phone: patient.phone, diagnosis: patient.diagnosis }); }, [patient]);
