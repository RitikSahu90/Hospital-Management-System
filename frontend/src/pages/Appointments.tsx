...
export default function Appointments() { const { user } = useAuth(); const canWrite = Boolean(user && ["ADMIN", "DOCTOR", "RECEPTIONIST", "PATIENT"].includes(user.role)); return <ApiResourcePage<Appointment> title="Appointments" fields={fields} load={getAppointments} canWrite={canWrite} create={(value) => createAppointment(value as AppointmentRequest)} />; }
